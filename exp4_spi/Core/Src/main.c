...
void spi_write_reg(uint8_t reg, uint8_t val) {
  ...
  // 1. 拉低, 选中芯片CS
  HAL_GPIO_WritePin(LIS2D_CS_GPIO_Port, LIS2D_CS_Pin, GPIO_PIN_RESET);
  // 2. 通过SPI写入数据
  HAL_SPI_Transmit(&hspi1, tx, 2, HAL_MAX_DELAY);
  // 3. 拉高, 取消选中CS
  HAL_GPIO_WritePin(LIS2D_CS_GPIO_Port, LIS2D_CS_Pin, GPIO_PIN_SET);
}
...
int main(void) {
  ...
  while (1) {
    ...
    // 读取WHO_AM_I 寄存器，0x0F 是WHO_AM_I 寄存器的地址
    uint8_t who_am_i = read_lis2dh12_reg(0x0F);
    // 输出检测结果
    printf("LIS2DH12 WHO AM I: 0x%02X ", who_am_i);
    // 验证设备ID并控制红色LED
    if (who_am_i == 0x33) {
      printf("- 传感器通信正常\r\n");
      // 红灯灭
